...
The catalog is broadened beyond those 2,850 rated titles with a small, git-tracked
TMDb-sourced supplement (`catalog_supplement/`, built by `src/movie_discovery.py`) —
see [Catalog broadening](#catalog-broadening) below. The real per-user-rated data
above is unaffected either way; the supplement only adds more titles to browse and
match against, none of them with ratings of their own.
...
flowchart TD
    ...
    L["TMDb catalog supplement\n(movie_discovery.py, one-time)"] -->|optional merge, no ratings| B
...
the **full 3,918-movie catalog with no negative sampling** (2,850 real, per-user-rated
titles plus the TMDb-sourced supplement — see [Catalog broadening](#catalog-broadening))
— harder than the sampled-candidate protocol common in papers/tutorials, so the
absolute numbers read lower, but they aren't inflated by an easier evaluation setup.
"Relevant" means the user explicitly marked a test movie liked (`rating == 1`),
matching this dataset's ternary scale.
...
| Content-based | 0.8760 | 0.7138 | 0.0010 | 0.0006 | 0.0010 | 0.0012 | 0.0031 | 0.0019 |
| SVD (collaborative) | **0.5607** | **0.4049** | 0.0122 | 0.0282 | 0.0204 | 0.0130 | **0.0667** | **0.0344** |
| Hybrid (weighted, α=0.6) | 0.6187 | 0.4938 | 0.0122 | 0.0283 | 0.0204 | 0.0127 | 0.0596 | 0.0327 |
...
The base dataset's 2,850 movies are real, per-user-rated data — genuine, but frozen at
whatever the original 2018 survey happened to cover. `src/movie_discovery.py` broadens
the catalog for browsing and onboarding (not ratings) by pulling additional
Indian-regional titles from TMDb's `/discover/movie` endpoint, one language at a time,
normalized into the exact schema the base loader already produces so no downstream code
needed to special-case them. A one-time run added **1,068 movies across 13 languages**
(2,850 → 3,918 total), each one only reachable through content/SVD-item similarity or
popularity — never a user's own collaborative signal, since none of them have real
ratings, same as ~55% of the base catalog already didn't. The output is a small,
git-tracked CSV (`catalog_supplement/`) rather than a live API call on every app
launch — same "real artifact, not silently regenerated" convention as
`results/metrics.json`.

One real data-quality bug surfaced before this ever shipped: TMDb's popularity ranking
surfaces hyped *unreleased* sequels right alongside actual releases, so an early run
picked up a handful of announced-but-not-out titles. Fixed by checking TMDb's own
`status == "Released"` field — recommending a movie nobody could have watched yet
doesn't belong in a recommendation catalog. Verified live: picking a supplement-only
title (e.g. "Jawan", released after the base dataset's 2018 collection window) in the
onboarding picker re-ranks results in real time to genuine neighbors, several of them
also supplement-only titles (see `CLAUDE.md`'s Phase 12 notes for the full story).
...
2. **The rating signal is sparser and more implicit.** ~20.5K ratings spread across a
   3,918-movie catalog (only 1,274 ever get a training rating — same real 2,850-movie
   rated set as before the TMDb catalog supplement, just a smaller share of a bigger
   denominator now) is an order of magnitude sparser than MovieLens 100K's ratio, and a
   ternary like/dislike/skip signal carries less information per rating than a 1–5 star
   scale. In that regime, a well-calibrated popularity prior is hard to beat for short
   ranked lists, and blending in a comparatively weak content signal mostly adds noise
   rather than complementary signal.
...
│   └── processed/
├── catalog_supplement/
│   └── tmdb_supplementary_movies.csv  # committed — TMDb-sourced catalog broadening, see below
├── results/
...
│   ├── movie_discovery.py # one-time TMDb catalog-broadening tool, see Catalog broadening
