...
Fixtures use a tiny 3-genre catalog (not the real 19-genre MovieLens set)
so similarity/weighted-rating values can be hand-computed and asserted
exactly, rather than just checking "it doesn't crash".
"""
...
class TestContentBasedRecommender:
    def test_recommend_ranks_unseen_movies_by_genre_similarity(self, movies_df, train_df):
        ...
        # user 1 rated movie 1 (Action, 5) and movie 2 (Action+Comedy, 4) -> both seen.
        # profile = 5*[1,0,0] + 4*[1,1,0] = [9, 4, 0]
        ...
        # sim(profile, movie3) = 4 / (||[9,4,0]|| * 1) = 4/9.849 = 0.4061
        # sim(profile, movie4) = 0
        # sim(profile, movie5) = 9 / (9.849 * sqrt(2)) = 0.6455
        # expected order: movie5 > movie3 > movie4
        assert recommended_ids == [5, 3, 4]

        profile_norm = math.sqrt(9**2 + 4**2)
        expected_sim_5 = 9 / (profile_norm * math.sqrt(2))
    ...
    def test_predict_scales_similarity_into_rating_range(self, movies_df, train_df):
        ...
        profile_norm = math.sqrt(9**2 + 4**2)
        expected_sim = 9 / (profile_norm * math.sqrt(2))
    ...
    def test_unknown_user_or_movie_returns_midpoint(self, movies_df, train_df):
        ...
        assert model.predict(999, 1) == pytest.approx((RATING_MIN + RATING_MAX) / 2)
        assert model.predict(1, 999) == pytest.approx((RATING_MIN + RATING_MAX) / 2)
...
class TestSVDRecommender:
    def test_predict_is_within_rating_bounds(self, train_df):
        ...
        for user_id, movie_id in [(1, 1), (2, 5), (1, 999), (999, 1), (999, 999)]:
    ...
    def test_unknown_user_and_item_returns_global_mean(self, train_df):
        ...
        assert model.predict(999, 999) == pytest.approx(train_df["rating"].mean(), abs=1e-9)
...
class TestPopularityRecommender:
    def test_weighted_rating_matches_hand_computed_formula(self, movies_df):
        ...
        # movie 1: n=3, mean=4; movie 2: n=1, mean=5. global_mean = (5+4+3+5)/4 = 4.25
        # m = median of [3, 1] = 2.0
        global_mean = 4.25
        m = 2.0
        expected_wr_movie1 = (3 / (3 + m)) * 4 + (m / (3 + m)) * global_mean
        assert model.predict(user_id=0, movie_id=1) == pytest.approx(expected_wr_movie1)

    def test_unrated_movie_falls_back_to_global_mean(self, movies_df, train_df):
        model = PopularityRecommender()
        model.fit(train_df, movies_df)

        assert model.predict(user_id=0, movie_id=99999) == pytest.approx(model._global_mean)
...
class TestColdStartRecommender:
    ...
    def test_unknown_user_with_zero_ratings_triggers_cold_start(self, cs_movies_df, cs_train_df):
        ...
        recs = wrapper.recommend_for_user(99999, n=3)
        assert recs
        assert all(r.source == "cold_start" for r in recs)
    ...
    def test_item_with_zero_ratings_falls_back_to_global_popularity(self, cs_movies_df, cs_train_df):
        ...
        # movie 99999 doesn't exist in the catalog at all -> same fallback path.
        assert wrapper.predict(2, 99999) == pytest.approx(standalone_popularity.predict(2, 99999))
